...
class SlotConsistencyTest(unittest.TestCase):
    ...
    def test_03_multi_value_slot(self):
        ...
        self.llm.extract_json.return_value = {
            "slot_candidates": [
                {"raw_key": "负载工具", "canonical_key": "payload", "raw_value": "水下成像系统,前视声呐系统", "normalized_value": ["水下成像系统", "前视声呐系统"], "confidence": 1.0}
            ],
            "list_mutations": [],
            "unresolved": []
        }
        self.dm.process("负载为水下成像系统和前视声呐系统")
        self.assertEqual(self.dm.slot_store.slots["payload"].value, ["水下成像系统", "前视声呐系统"])
        assert_ssot_consistency(self, self.dm)
