...
from src.slot_store import SlotStore, SnapshotValidationError
...
def test_slot_store_snapshot_schema_v2(dm):
    ...
    assert snap["validation"] == store.validation_result
    assert snap["validation_acknowledgements"] == store.validation_acknowledgements

    # 新建 SlotStore 恢复
    new_store = SlotStore.from_snapshot(snap, kb=dm.kb)
    assert new_store.validation_result == store.validation_result
    assert new_store.validation_acknowledgements == store.validation_acknowledgements
...
def test_soft_warning_acknowledgement_fingerprint_invalidation(dm):
    ...
    assert "不满足" in reply or "不符合" in reply or "重" in reply or "确认" in reply
