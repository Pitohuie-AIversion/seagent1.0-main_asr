...
class BlockerPriorityTransitionsTest(unittest.TestCase):
    ...
    # 1. collecting + soft -> blocked_soft
    ...
    # 2. collecting + hard -> blocked_hard
    ...
    # 8. blocked_hard + hard解除 + soft -> blocked_soft
    ...
    # 11. C028 单独触发 -> blocked_soft (真实 oilfield context 接入)
    def test_11_real_oilfield_c028_triggers_blocked_soft(self):
        ...
        dm._run_constraint_check({"start_point"})
        self.assertEqual(dm.phase, "blocked_soft")
        violations = dm._blocking_violations
        self.assertTrue(any(v.constraint_id == "C028" for v in violations))
