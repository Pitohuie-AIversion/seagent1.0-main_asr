...
for item_key, new_aliases in alias_additions.items():
    if item_key in catalog:
        existing = catalog[item_key].get('aliases', [])
        for a in new_aliases:
            if a not in existing:
                existing.append(a)
        catalog[item_key]['aliases'] = existing
...
# To preserve structure and comments cleanly:
# Let's write out custom dumper or structured formatting
...
print("update_assets_final complete!")
