...
4. **收集策略（按 task schemas 顺序，每轮固定以 3 个关联字段为单位收集）**：
   ...
   - **以 3 个关联字段为单位收集（极其重要）**：
     * **固定收集单位**：必须严格按照后端提供的“待收集字段列表”（json task schemas 顺序），从列表顶部首个未写入字段开始，**固定提取顶部连续 3 个相关联的缺失字段**（若剩余不足 3 个，则提取全部剩余缺失字段）作为本轮的收集目标组合。
     * **精简利落直奔主题**：回复必须简洁明了、重点突出，直接展示已写入项（如有）并紧接着清晰提出这 3 个关联字段的补充请求，切忌冗长废话或铺垫开场白。
     * **顺延推进机制**：一旦用户在本轮提供了有效信息并成功写入了字段，后端待收集列表会自动更新，下一轮继续顺延提取后续未写入的前 3 个关联字段进行收集。
   ...
   - **设备层级依赖顺序**：设备选型具有自然的逐级依赖关系（类别 -> 系列 -> 型号 -> 编号）。
     ...
     * 在设备依赖满足的前提下，设备字段与同轮其他平行的任务字段合并凑足 3 个关联字段收集单位。
...
def build_responder_messages(
    ...
    # ── 缺失字段描述（含允许值提示）─────────────────────────────────────────
    if missing_fields:
        missing_lines = ["  （以下为待收集字段列表，已按 json task schemas 顺序排列。请严格从列表顶部首个未写入字段开始，固定截取顶部连续 3 个相关联缺失字段进行集中追问，回复保持精炼）："]
