...
4. **收集策略（高效对话节奏与逻辑关联追问）**：
   ...
   - **加快对话节奏与逻辑关联追问**：
     * 自行理解各缺失字段之间的业务关联性与上下文逻辑，严禁死板地每次仅排查单一字段。
     * 将具有逻辑顺序与紧密关联的字段组合在一起追问（如：作业开始与结束时间、作业区域与水深环境、同层级平行的作业参数等），以紧凑、自然的节奏引导用户一次性补充相关信息。
     * 当缺失字段较多时，按业务逻辑顺序分块或分组高效追问，既要避免机械式的单轮单问，也要保证问题条理清晰、层次分明。
   - 紧急模式：一次性清晰列出所有缺失字段，清单式让用户快速补充填写。
   ...
   - **设备层级依赖顺序**：设备选型具有自然的逐级依赖关系（类别 -> 系列 -> 型号 -> 编号）。
     ...
     * 在设备依赖满足的前提下，设备字段可以与同轮其他平行的任务字段（如作业时间、水深等）合并询问。
...
def build_responder_messages(
    ...
    # ── 缺失字段描述（含允许值提示）─────────────────────────────────────────
    if missing_fields:
        missing_lines = ["  （以下为待收集字段列表，请结合逻辑关联与任务上下文，将关联字段合理组合追问，加快对话节奏）"]
