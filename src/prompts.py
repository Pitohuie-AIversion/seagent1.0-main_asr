...
4. **收集策略（按 task schemas 顺序，每轮 1-3 个关联字段精简追问）**：
   ...
   - **精简回复与追问数量控制（极其重要）**：
     * **回复必须精简利落、重点突出**：切忌冗长废话或大段开场白，直接说明写入结果并提出问题，让用户一目了然抓住重点。
     * **每次对话仅追问 1 ~ 3 个相关联字段**：严禁单轮一次性询问 4 个或更多字段，严禁把全部缺失列表堆砌给用户。
     * **严格按 task schemas 顺序引导**：必须严格按照后端提供的“待收集字段列表”（json task schemas）顺序从顶部开始引导。提取列表中当前最靠前未填写的 1 ~ 3 个逻辑相关联的缺失字段进行追问（例如：作业时间范围[开始时间与结束时间]、作业区域与水深[油田名、坐标、水深]、设备型号与载荷等）。
     * **顺延补充机制**：一旦用户在本轮提供了有效信息并成功写入某些字段，下一轮应根据最新更新的待收集字段列表，继续顺延追问后续尚未写入的 1 ~ 3 个缺失字段。
   ...
   - **设备层级依赖顺序**：设备选型具有自然的逐级依赖关系（类别 -> 系列 -> 型号 -> 编号）。
     ...
     * 在设备依赖满足的前提下，设备字段可以与同轮其他平行的任务字段合并询问（总数不超过 3 个）。
...
def build_responder_messages(
    ...
    # ── 缺失字段描述（含允许值提示）─────────────────────────────────────────
    if missing_fields:
        missing_lines = ["  （以下为待收集字段列表，已按 json task schemas 顺序排列。请严格从列表顶部首个未写入字段开始，提取 1~3 个逻辑关联缺失字段进行精简追问，避免冗长回复）："]
