...
### Refactored — DDD 领域子包模块化与根目录 Shims 彻底清除（2026-09-20）

- **`src/` 扁平文件领域分层**：将原先平铺在 `src/` 根目录的 48 个 Python 文件按 DDD 领域驱动高内聚原则彻底划分为 12 个领域子包（`asr/`, `temporal/`, `slots/`, `validation/`, `dispatch/`, `extraction/`, `session/`, `handlers/`, `knowledge/`, `types/`, `utils/`, `web/`）。
- **彻底消除 40 个转发垫片（Shims）**：物理删除 `src/` 根目录下遗留的 40 个 shim 转发文件，根目录收敛为仅保留 10 个系统总门面与全局基础设施文件（`dialogue_manager.py`, `llm_client.py`, `knowledge_retriever.py`, `constants.py` 等）。
- **全仓库依赖与绝对导入对齐**：全量更新 160+ 个模块及测试文件的导入路径，统一指向正规的 `src.<subpackage>.<mod>`；修复 `src/handlers/` 下 17 处原指向根目录的两层相对导入。
- **子包按需延迟加载机制**：各子包 `__init__.py` 采用 PEP 562 延迟加载机制（`__getattr__`），杜绝相互引用产生的循环导入风险，优化冷启动开销。
- **全量回归与端到端真实测试通过**：`pytest -q` 全量 2631 个测试用例（530 subtests）100% 通过（0 失败）；并完成基于真实 Qwen3.5-9B 与 ASR 模型的端到端多轮用户交互体验与任务原子落盘验收。

### Added — Phase 2：LLM 语义权威 & 约束驱动机器人候选（2026-08-13）

- **LLM 语义权威路由（ADR-005 第二阶段）**：[src/session/interaction_plan.py](file:///root/mzy/seagent1.0-main_asr/src/session/interaction_plan.py) 引入 `InteractionPlan`，`operation`（READ / WRITE / CONTROL / CLARIFY）成为每轮路由的唯一语义权威字段；后端确定性代码不再根据业务关键词改变操作类型。
...
- **可见来源选择追踪**：新增 [src/slots/visible_selection_provenance.py](file:///root/mzy/seagent1.0-main_asr/src/slots/visible_selection_provenance.py)，严格校验编号候选选择的可见来源，阻止基于隐藏候选顺序的误写。
- **Grounded 字段推荐逻辑**：引入 `recommend` 关系，`DialogueManager` 基于已确认任务条件给出有证据的字段推荐，不以列表第一项冒充智能推荐。
- **坐标解析确定性增强**：[src/extraction/coord_parser.py](file:///root/mzy/seagent1.0-main_asr/src/extraction/coord_parser.py) 重构，保证坐标解析结果与输入格式无关的唯一确定性。
...
- **TaskPatch 引擎**：新增 [src/slots/task_patch.py](file:///root/mzy/seagent1.0-main_asr/src/slots/task_patch.py)，支持任务参数字段级补丁操作，不覆盖整体 SlotStore。
- **任务请求守卫（TaskRequestGuard）**：新增 [src/validation/task_request_guard.py](file:///root/mzy/seagent1.0-main_asr/src/validation/task_request_guard.py)，防止同轮内多任务意图误触发。
...
- **WRITE / QUERY 意图分路路由**：新增 [src/session/intent_router.py](file:///root/mzy/seagent1.0-main_asr/src/session/intent_router.py) (`IntentRouter`, `IntentRouteResult`)，将用户输入解耦为写任务状态 (`WRITE`) 与读知识/状态 (`QUERY`) 两个独立通道。
- **SlotStore 状态中心与事务机制**：引入 [src/slots/slot_store.py](file:///root/mzy/seagent1.0-main_asr/src/slots/slot_store.py) (`SlotStore`, `Slot`) 作为 Single Source of Truth，支持版本号追踪、快照导出/恢复与回滚事务。
...
- **TaskIntent 原子安全持久化**：在 [src/dispatch/task_intent_builder.py](file:///root/mzy/seagent1.0-main_asr/src/dispatch/task_intent_builder.py) 中引入暂存区 Staging 机制、跨进程排他锁 `TaskPublishLock` 与原子提交 `_atomic_commit_noreplace`，防止落盘并发覆盖与非法篡改。
...
- **Responder 消息生成重构**：[src/dispatch/output_builder.py](file:///root/mzy/seagent1.0-main_asr/src/dispatch/output_builder.py) 消除固定关键词依赖，由模型语义权威驱动结构化回复生成。
...
- **Path Traversal 与 TaskIntent 篡改防御**：在 [src/dispatch/task_intent_builder.py](file:///root/mzy/seagent1.0-main_asr/src/dispatch/task_intent_builder.py) 中严格校验目标路径、符号链接与 PID 拥有权，禁用硬删与强制替换。
