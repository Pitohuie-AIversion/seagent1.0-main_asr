...
PYTHON ?= $(shell which python3 || echo /root/miniconda3/envs/seagent/bin/python)
...
help:
	...
	@echo "  make test   - 在当前 Python 环境下执行完整单元测试套件"
...
status:
	@echo "🔍 检查端口 8890 监听状态:"
	@ss -tlnp 'sport = :8890' 2>/dev/null || fuser -n tcp 8890 2>/dev/null || echo "端口 8890 当前没有被监听 (服务可能未启动)。"
